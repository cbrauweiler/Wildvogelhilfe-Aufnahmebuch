...
<table id="noprint" class="striped centered">
	...
	<?php
		...
		// Vogelarten	
		$query = "SELECT `vogel_art`, sum(vogel_anzahl) as summe_anflugtrauma FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `datum` like '$statyear%' AND `vogel_anflugtrauma`='1' GROUP BY `vogel_art` ORDER BY `vogel_art`";
		if($result = mysqli_query($mysqli, $query)) {
			while($row = $result->fetch_array(MYSQLI_BOTH)) {
				...
				// Ästling
				$query3 = "SELECT sum(vogel_anzahl) as summe_anzahl, sum(vogel_weitergeleitet) as summe_weitergeleitet, sum(vogel_ausgewildert) as summe_ausgewildert, sum(vogel_euthanasiert) as summe_euthanasiert, sum(vogel_verstorben) as summe_verstorben FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `datum` like '$statyear%' AND `vogel_art`='$vogel_art' AND `vogel_stadium`='aestling' AND `vogel_anflugtrauma`='1'";
				if($result3 = mysqli_query($mysqli, $query3)) {
					while($row3 = $result3->fetch_array(MYSQLI_BOTH)) {
				...
				// Juvenil
				$query4 = "SELECT sum(vogel_anzahl) as summe_anzahl, sum(vogel_weitergeleitet) as summe_weitergeleitet, sum(vogel_ausgewildert) as summe_ausgewildert, sum(vogel_euthanasiert) as summe_euthanasiert, sum(vogel_verstorben) as summe_verstorben FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `datum` like '$statyear%' AND `vogel_art`='$vogel_art' AND `vogel_stadium`='juvenil' AND `vogel_anflugtrauma`='1'";
				if($result4 = mysqli_query($mysqli, $query4)) {
					while($row4 = $result4->fetch_array(MYSQLI_BOTH)) {
				...
				// Adult
				$query5 = "SELECT sum(vogel_anzahl) as summe_anzahl, sum(vogel_weitergeleitet) as summe_weitergeleitet, sum(vogel_ausgewildert) as summe_ausgewildert, sum(vogel_euthanasiert) as summe_euthanasiert, sum(vogel_verstorben) as summe_verstorben FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `datum` like '$statyear%' AND `vogel_art`='$vogel_art' AND `vogel_stadium`='adult' AND `vogel_anflugtrauma`='1'";
				if($result5 = mysqli_query($mysqli, $query5)) {
					while($row5 = $result5->fetch_array(MYSQLI_BOTH)) {
