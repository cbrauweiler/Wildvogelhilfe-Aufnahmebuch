...
<?php
	...
	#$query = "SELECT * FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `papierkorb`='0' AND `vogel_verstorben`>0 AND `vogel_anzahl`!=vogel_ausgewildert+vogel_euthanasiert+vogel_verstorben+vogel_weitergeleitet ORDER BY `datum` DESC";
	$query = "SELECT * FROM `aufnahmebuch` WHERE `mandant`='$session_mandant' AND `papierkorb`='0' AND `vogel_verstorben`>0 ORDER BY `datum` DESC";
...
?>
	<tr>
		<td colspan="2" class="listsum"><strong>Gesamt Verstorben:</strong></td>
